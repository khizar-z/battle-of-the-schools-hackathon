...
When `OPENAI_API_KEY` is configured, the server uses GPT-5 Mini Structured Outputs to parse shopping intent; any model/API failure automatically falls back to the local parser. Recipe data is persisted locally in `data/site-recipes.json` and is ignored by Git. See [architecture](docs/architecture.md) and the [demo runbook](docs/demo.md).
